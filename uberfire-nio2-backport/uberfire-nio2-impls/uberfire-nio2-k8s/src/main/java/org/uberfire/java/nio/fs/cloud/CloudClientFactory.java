...
public interface CloudClientFactory {
    ...
    default <T extends KubernetesClient, R> Optional<R> executeCloudFunction(Function<T, R> func, Class<T> type) throws IOException {
        final Logger logger = LoggerFactory.getLogger(CloudClientFactory.class);
        R result = null;
        try (T client = (T) (type == OpenShiftClient.class ? createOpenShiftClient() : createKubernetesClient())) {
            result = func.apply(client);
        } catch (UnsupportedOperationException uoe) {
            logger.error("Executing unsupported cloud function: ", uoe);
            throw uoe;
        } catch (IllegalStateException ise) {
            logger.error("Executing cloud function failed - Missing required configuration: ", ise);
            throw ise;
        } catch (Exception e) {
            logger.error("Executing cloud function failed: ", e);
